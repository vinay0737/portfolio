    ...
    >
      ...
      {backgroundShapes.map((shape, index) => (
        <motion.div
          ...
          animate={{
            x: mouseParallax.x * (index + 1) * 0.5,
            y: mouseParallax.y * (index + 1) * 0.5,
            scale: [1, 1.1, 1],
          }}
          transition={{
            scale: {
      ...
      <motion.div
        className="max-w-3xl relative z-10 text-white"
      ...
      >
        <motion.div
          ...
          className="mb-3 inline-block py-1 px-3 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 rounded-full backdrop-blur-md"
        >
          <span className="text-sm font-medium text-blue-200">BlockChain Developer & Data Engineer</span>
        </motion.div>
        ...
        <motion.p 
          variants={itemVariants}
          className="text-xl sm:text-2xl text-gray-200 mb-8 leading-relaxed drop-shadow-md font-light"
        >
          I'm a passionate{" "}
        ...
        <motion.div 
          variants={itemVariants}
          className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-5 justify-center"
        >
          <motion.a
            href="/Vinay's_CV.pdf"
            download
            className="relative overflow-hidden group bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-8 py-4 rounded-lg font-semibold shadow-xl hover:shadow-2xl transition duration-300"
          ...
          <motion.a
            href="#projects"
            className="relative overflow-hidden group border-2 border-blue-400 text-blue-400 hover:text-white px-8 py-4 rounded-lg font-semibold transition duration-300"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.98 }}
          >
            <motion.span 
        ...
        <motion.div
          variants={itemVariants}
          className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-12 max-w-xl mx-auto"
        >
          {[
      ...
      {/* Tech stack icons - subtle floating animation */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        {["HyperLedger Fabric", "Mern Stack", "Python", "Azure", "MongoDB"].map((tech, i) => (
          ...
          >
            <div className="w-3 h-3 rounded-full bg-blue-400 mb-2 opacity-70" />
